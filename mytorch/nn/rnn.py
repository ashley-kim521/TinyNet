...
class RNNUnit(Module):
    ...
    def forward(self, input, hidden = None):
        ...
        h_t = self.act(input @ self.weight_ih.T() + self.bias_ih + hidden @ self.weight_hh.T() + self.bias_hh)
...
class TimeIterator(Module):
    ...
    def forward(self,input,hidden = None):
        ...
        for p in range(len(batch_sizes)):
            effective_batch_size = batch_sizes[p]
            x = data[counter:counter+effective_batch_size]
            hidden_state = self.unit(x, previous_hidden_state)
            list_of_hidden.append(hidden_state)
            previous_hidden_state = hidden_state

            if p == (len(batch_sizes)-1):
                list_of_last_hidden.append(hidden_state)
            else:
                new = batch_sizes[p+1]
                list_of_last_hidden.append(hidden_state[new:])
                previous_hidden_state = hidden_state[:new]
            counter = counter + effective_batch_size

        packed_tensor = tensor.cat(list_of_hidden, dim=0)
        packed_object = PackedSequence(packed_tensor, sorted_indices, batch_sizes) 
        tensor_last_list = tensor.cat(list_of_last_hidden[::-1], dim=0)

        return (packed_object, tensor_last_list)
...
class RNN(TimeIterator):
    ...
    def __init__(self, input_size, hidden_size, nonlinearity = 'tanh' ):
        ...
        # self.unit = RNNUnit(input_size,hidden_size,nonlinearity)    